...
Triangulator::Triangulator(std::vector<MapPoint>& input) 
{
	...
	for (int i = 0; i < n; i++) 
	{
		const float x = input[2 * i];
		const float y = input[2 * i + 1];
		if (x < minX) minX = x;
		if (y < minY) minY = y;
		if (x > maxX) maxX = x;
		if (y > maxY) maxY = y;
	...
	for (int i = 0; i < n; i++) 
	{
		const float d = dist(cx, cy, input[2 * i], input[2 * i + 1]);
		if (d < minDist) 
		{
			i0 = i;
			minDist = d;
	...
	for (int i = 0; i < n; i++) 
	{
		...
		const float d = dist(input[2 * i0], input[2 * i0 + 1], input[2 * i], input[2 * i + 1]);
		if (d < minDist && d > 0) 
		{
			i1 = i;
			minDist = d;
