...
function addGhostStructure({_r_vertex, _s_start_r, _s_opposite_s}) {
    ...
    let r_newvertex = _r_vertex.concat([[10, 10]]);
    ...
    for (let i = 0;
        ...
        const index = s_next_s(s);
        s_newstart_r[ghost_s] = s_newstart_r[index];
        ...
        let next_s = s_next_s(s);
        let next_tri = s_newstart_r[next_s];
        let next_s_s = r_unpaired_s[next_tri];
        s = r_unpaired_s[s_newstart_r[s_next_s(s)]]
    }
...
function createMesh({spacing=Infinity, points=[], random=Math.random}) {
    ...
    let ghost_start = graph._s_start_r.length;

    checkPointInequality(graph);
    checkTriangleInequality(graph);
    
    graph = addGhostStructure(graph);
/*     for(let i = ghost_start; i < graph._s_start_r.length; ++i)
    {
        console.log(graph._s_start_r[i]);
    } */
